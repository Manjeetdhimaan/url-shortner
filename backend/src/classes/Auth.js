...
class Auth {
    static async login(username, password) {
        if (!UserManager.usernameExists(username) || !UserManager.getFromEmail(username)) {
        ...
        let user = await UserManager.getFromUsername(username);

        if (!user) {
            user = await UserManager.getFromEmail(username);
            if (!user) {
                return {
                    status: false,
